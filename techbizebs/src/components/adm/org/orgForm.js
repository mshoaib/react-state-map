...
/* **************** Update API **************** */
    API.put(`/adm/update-org/${Organization_ID}`, data, {
      header: {
        "Content-Type": "application/json"
      }
    })
      ...
      <MDBRow>
        <MDBCol>
          <form  onSubmit={handleSubmit(onSubmit)}>
            ...
            <p style={{color : "#113C87"}} className="h5 text-left py-1">Organization </p>

            
            <MDBInput autoComplete="off" style={{marginBottom:"2px" }} name="Organization_Name" valueDefault = {props.type === "update" ? props.record.Organization_Name : ""} inputRef={register({ required: true, maxLength: 255 })   }  label="Organization Name"  group type="text"  validate error="wrong" success="right" />
            {errors.Organization_Name && errors.Organization_Name.type === "required" &&  <p className = "form_error"> <i className="fas fa-exclamation-triangle"></i> This field is required</p>}
            {errors.Organization_Name && errors.Organization_Name.type === "maxLength" && <p className = "form_error"> Maximum Length Allowed is 250 </p>}

            <MDBRow>
            <MDBCol>
            <MDBInput autoComplete="off" style={{marginBottom:"2px" }} name="Organization_Short_Name" valueDefault = {props.type === "update" ? props.record.Organization_Short_Name : ""} inputRef={register({ required: true, maxLength: 255 })   }  label="Short Code"  group type="text"  validate error="wrong" success="right" />
            {errors.Organization_Short_Name && errors.Organization_Short_Name.type === "required" &&  <p className = "form_error"> <i className="fas fa-exclamation-triangle"></i> This field is required</p>}
            {errors.Organization_Short_Name && errors.Organization_Short_Name.type === "maxLength" && <p className = "form_error"> Maximum Length Allowed is 250 </p>}
            </MDBCol>
            <MDBCol>
            
            <MDBInput autoComplete="off" style={{marginBottom:"3px" }} name ="Organization_Desc" valueDefault = {props.type === "update" ? props.record.Organization_Desc : ""} inputRef={register({ required: true, maxLength: 255 })   }  label="Description"  group type="text"  validate error="wrong" success="right" />
            {errors.Organization_Desc  && errors.Organization_Desc.type === "required" && <p className = "form_error"> <i className="fas fa-exclamation-triangle"></i> This field is required</p>}
            {errors.Organization_Desc  && errors.Organization_Desc.type === "maxLength" && <p className = "form_error"> Maximum Length Allowed is 500 </p>}
